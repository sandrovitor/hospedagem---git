...
$(document).ready(function() {
	$("#panel1").on("hide.bs.collapse", function(){
		$("#panel1-btn-link").html('[+] Mostrar');
		$("#panel1-qtd").fadeIn('fast');
		$("#panel1-revisar").fadeIn('fast');
	});
	$("#panel1").on("show.bs.collapse", function(){
		$("#panel1-btn-link").html('[-] Esconder');
		$("#panel1-qtd").fadeOut('fast');
		$("#panel1-revisar").fadeOut('fast');
	});

	$("#panel2").on("hide.bs.collapse", function(){
		$("#panel2-btn-link").html('[+] Mostrar');
		$("#panel2-qtd").fadeIn('fast');
		$("#panel2-revisar").fadeIn('fast');
	});
	$("#panel2").on("show.bs.collapse", function(){
		$("#panel2-btn-link").html('[-] Esconder');
		$("#panel2-qtd").fadeOut('fast');
		$("#panel2-revisar").fadeOut('fast');
	});
});
</script>
			...
			<ul class="nav navbar-nav">
				...
				<li class="dropdown">
					...
					<ul class="dropdown-menu">
						...
						<li class="active"><a href="formulario.consulta.php"><span class="glyphicon glyphicon-search"></span> &nbsp; Consultar</a></li>
						...
						<div class="panel-group">
							<?php 
												if($_SESSION['nivel'] == 1) { // Solicitante de hospedagem
													$def = $pdo->query('SELECT DISTINCT peh.id FROM peh LEFT JOIN cidade ON peh.congregacao_cidade_id = cidade.id WHERE cidade.solicitante_id = '.$_SESSION['id']);
													
												} else if ($_SESSION['nivel'] == 10) { // Responsável pela hospedagem
													$def = $pdo->query('SELECT DISTINCT peh.id FROM peh LEFT JOIN cidade ON peh.congregacao_cidade_id = cidade.id WHERE cidade.resp_id = '.$_SESSION['id']);
														
												} else if ($_SESSION['nivel'] == 20) { // Administrador
													$def = $pdo->query('SELECT DISTINCT peh.id FROM peh LEFT JOIN cidade ON peh.congregacao_cidade_id = cidade.id WHERE 1');
													
												}
												
												//TOTAL DE PEDIDOS
												$total_pedidos = $def->rowCount();
												
												if($_SESSION['nivel'] == 1) { // Solicitante de hospedagem
												    $def = $pdo->query('SELECT DISTINCT peh.id FROM peh LEFT JOIN cidade ON peh.congregacao_cidade_id = cidade.id WHERE peh.revisar = 1 AND cidade.solicitante_id = '.$_SESSION['id']);
												    
												} else if ($_SESSION['nivel'] == 10) { // Responsável pela hospedagem
												    $def = $pdo->query('SELECT DISTINCT peh.id FROM peh LEFT JOIN cidade ON peh.congregacao_cidade_id = cidade.id WHERE peh.revisar = 1 AND cidade.resp_id = '.$_SESSION['id']);
												    
												} else if ($_SESSION['nivel'] == 20) { // Administrador
												    $def = $pdo->query('SELECT DISTINCT peh.id FROM peh LEFT JOIN cidade ON peh.congregacao_cidade_id = cidade.id WHERE peh.revisar = 1');
												    
												}
												
												// PEDIDOS A REVISAR
												if($def->rowCount() > 0) {
												    $peh_revisar = '<span id="panel1-revisar" class="label label-danger" title="PEDIDOS ESPERANDO REVISÃO!" data-toggle="tooltip"><span class="glyphicon glyphicon-alert"></span> &nbsp; '.$def->rowCount().'</span>';
												} else {
												    $peh_revisar = '<span id="panel1-revisar" class="label label-success" title="PEDIDOS ESPERANDO REVISÃO!" data-toggle="tooltip">0</span>';
												}
							?>
							...
							<div class="panel panel-default">
								<div class="panel-heading">
									...
									<span id="panel1-qtd" class="label label-primary"><?php echo $total_pedidos;?></span>
									<?php echo $peh_revisar;?>
									<a class="pull-right" id="panel1-btn-link" data-toggle="collapse" data-target="#panel1">[+] Mostrar</a>
								</div>
...
DADOS;
														...
														if($abc->rowCount() > 0) { // Linhas encontradas no banco
															while($lin = $abc->fetch(PDO::FETCH_OBJ)) {
																...
																if($lin->revisar == 0) {
																	$info .= '<br><span id="peh_rev-'.$lin->id.'" class="label label-default" data-toggle="tooltip" title="TUDO OK"><span class="glyphicon glyphicon-alert"></span> REV</span>';
																} else {
																	$info .= '<br><span id="peh_rev-'.$lin->id.'" class="label label-danger" data-toggle="tooltip" title="REVISAR!"><span class="glyphicon glyphicon-alert"></span> REV</span>';
																}
...
DADOS;
																...
																if(($_SESSION['nivel'] == 10 || $_SESSION['nivel'] == 20)) {
																	...
																	$bot .= <<<DADOS
	<button onclick="revisarForms('peh', $lin->id, this)" class="btn btn-sm btn-warning" title="Pedir revisão" data-toggle="tooltip" $dis><span class="glyphicon glyphicon-comment"></span></button>
DADOS;
																} else { // Se for nivel 1
...
DADOS;
							...
							<?php if($_SESSION['nivel'] != 1) {
										if ($_SESSION['nivel'] == 10) { // Responsável pela hospedagem
											$abc = $pdo->query('SELECT DISTINCT fac.id FROM fac LEFT JOIN cidade ON fac.cidade = cidade.id WHERE cidade.hospedeiro = 1 AND cidade.resp_id = '.$_SESSION['id']);
											
										} else if ($_SESSION['nivel'] == 20) { // Administrador
											$abc = $pdo->query('SELECT DISTINCT fac.id FROM fac LEFT JOIN cidade ON fac.cidade = cidade.id WHERE cidade.hospedeiro = 1');
											
										}
										$total_acomod = $abc->rowCount();
										
										
										
										if ($_SESSION['nivel'] == 10) { // Responsável pela hospedagem
										    $abc = $pdo->query('SELECT DISTINCT fac.id FROM fac LEFT JOIN cidade ON fac.cidade = cidade.id WHERE fac.revisar = 1 AND cidade.hospedeiro = 1 AND cidade.resp_id = '.$_SESSION['id']);
										    
										} else if ($_SESSION['nivel'] == 20) { // Administrador
										    $abc = $pdo->query('SELECT DISTINCT fac.id FROM fac LEFT JOIN cidade ON fac.cidade = cidade.id WHERE fac.revisar = 1 AND cidade.hospedeiro = 1');
										    
										}
										
										// PEDIDOS A REVISAR
										if($abc->rowCount() > 0) {
										    $fac_revisar = '<span id="panel2-revisar" class="label label-danger" title="FORMULÁRIOS ESPERANDO REVISÃO!" data-toggle="tooltip"><span class="glyphicon glyphicon-alert"></span> &nbsp; '.$abc->rowCount().'</span>';
										} else {
										    $fac_revisar = '<span id="panel2-revisar" class="label label-success" title="FORMULÁRIOS ESPERANDO REVISÃO!" data-toggle="tooltip">0</span>';
										}
							...
							<div class="panel panel-default">
								<div class="panel-heading">
									...
									<span id="panel2-qtd" class="label label-primary"><?php echo $total_acomod; ?></span>
									<?php echo $fac_revisar;?>
									<a class="pull-right" id="panel2-btn-link" data-toggle="collapse" data-target="#panel2">[+] Mostrar</a>
								</div>
									...
									<div class="panel-body">
										<table class="table table-condensed">
											<tbody>
												...
												<?php 
												if ($_SESSION['nivel'] == 10) { // Responsável pela hospedagem
													$abc = $pdo->query('SELECT DISTINCT fac.id, fac.cidade AS cidade_id, fac.revisar, cidade.cidade, cidade.estado FROM fac LEFT JOIN cidade ON fac.cidade = cidade.id WHERE cidade.hospedeiro = 1 AND cidade.resp_id = '.$_SESSION['id'].' ORDER BY cidade.estado ASC, cidade.cidade ASC');
													
												} else if ($_SESSION['nivel'] == 20) { // Administrador
													$abc = $pdo->query('SELECT DISTINCT fac.id, fac.cidade AS cidade_id, fac.revisar, cidade.cidade, cidade.estado FROM fac LEFT JOIN cidade ON fac.cidade = cidade.id WHERE cidade.hospedeiro = 1 ORDER BY cidade.estado ASC, cidade.cidade ASC, fac.id ASC');
...
DADOS;
														...
														if($reg->revisar == 0) {
														    $info .= '<br><span id="fac_rev-'.$reg->id.'" class="label label-default" data-toggle="tooltip" title="TUDO OK"><span class="glyphicon glyphicon-alert"></span> REV</span>';
														} else {
														    $info .= '<br><span id="fac_rev-'.$reg->id.'" class="label label-danger" data-toggle="tooltip" title="REVISAR!"><span class="glyphicon glyphicon-alert"></span> REV</span>';
														}
...
DADOS;
														...
														if($_SESSION['nivel'] == 20) {
														    ...
														    $bot .= <<<DADOS
	<button onclick="revisarForms('fac', $reg->id, this)" class="btn btn-sm btn-warning" title="Pedir revisão" data-toggle="tooltip" $dis><span class="glyphicon glyphicon-comment"></span></button>
DADOS;
														}
														...
														echo <<<DADOS
												<tr id="fac-$reg->id">
													<td style="vertical-align: middle"><a onclick="facCarrega($reg->id,0)" data-toggle="collapse" data-target="#panel_pedidos" class="btn btn-link">Acomod. Nº.: $reg->id</a></td>
													<td>$info</td>
													<td>$bot</td>
												</tr>
DADOS;
