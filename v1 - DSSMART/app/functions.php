	...
	if($abc->rowCount() > 0) {
		while($reg = $abc->fetch(PDO::FETCH_OBJ)) {
			...
			if($reg->revisar == 1 && ($_SESSION['nivel'] == 1 || $_SESSION['nivel'] == 20)) {
				...
				$revisar = '&nbsp; <a href="formulario.edita.php?tipo=peh&pehid='.$reg->id.'&token='.$token.'" target="_blank" class="btn btn-sm btn-success" title="Editar" data-toggle="tooltip"><span class="glyphicon glyphicon-edit"></span></a>';
			} else {
				$revisar = '';
			}
...
function vincularForms($pehid, $facid) {
	...
	if($peh->fac_id == 0) {
		...
		// PEDIDO JÁ FOI VINCULADO COM OUTRA ACOMODAÇÃO.
		exit('Hospedagem vinculada com outra acomodação.');
	}
...
switch($_REQUEST['funcao']) {
	...
	case 'vincularForms':
		vincularForms($_REQUEST['pehid'], $_REQUEST['facid']);
		break;
	...
	case 'desvAction':
		desvAction($_REQUEST['tipo'], $_REQUEST['id']);
		break;
